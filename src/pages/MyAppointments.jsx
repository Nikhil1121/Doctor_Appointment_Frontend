...
const MyAppointments = () => {
  ...
  const handlePayOnline = (item) => { setSelectedAppointment(item); setPaymentDone(false); setShowPayModal(true); };
  ...
  const getStatusBadge = (item) => {
    if (item.cancelled) return (
      <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl p-3">
        ...
        <div><p className="text-red-500 font-semibold text-sm">Appointment Cancelled</p><p className="text-red-400 text-xs">This appointment has been cancelled</p></div>
      </div>
    );
    if (item.isCompleted) return (
      <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl p-3">
        <span className="text-2xl">✅</span>
        <div><p className="text-green-600 font-semibold text-sm">Appointment Completed</p><p className="text-green-500 text-xs">Your appointment is completed</p></div>
      </div>
    );
    if (item.isConfirmed) return (
      <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl p-3">
        <span className="text-2xl">🎉</span>
        <div><p className="text-blue-600 font-semibold text-sm">Appointment Confirmed!</p><p className="text-blue-500 text-xs">{slotDateFormat(item.slotDate)} at {item.slotTime}</p></div>
      </div>
    );
    return (
      <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl p-3">
        <span className="text-2xl">⏳</span>
        <div><p className="text-yellow-600 font-semibold text-sm">Pending Confirmation</p><p className="text-yellow-500 text-xs">Waiting for admin to confirm</p></div>
      </div>
    );
  };
              ...
              <>
                ...
                <div className="flex justify-center mb-4">
                  <div className="border-4 border-primary rounded-2xl p-2">
                    <img src="https://res.cloudinary.com/ddslcdypq/image/upload/v1/scanner" alt="PhonePe QR" className="w-52 h-52 object-contain" />
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-gray-500 space-y-1">
                    ...
                    <p>💰 Fees: <span className="font-semibold text-primary">₹{item.amount}</span>{item.payment && <span className="ml-2 text-green-500 text-xs">✅ Paid</span>}</p>
                  </div>
                </div>
                <div className="flex flex-col gap-2 sm:w-44">
                  {!item.cancelled && !item.isCompleted && (
                    <>
                      {!item.payment ? (
                        <button onClick={() => handlePayOnline(item)} className="w-full py-2 bg-primary text-white rounded-xl text-sm font-medium hover:opacity-90 transition-all">💳 Pay Online</button>
                      ) : (
                        <button className="w-full py-2 bg-green-50 text-green-500 border border-green-300 rounded-xl text-sm cursor-default">✅ Paid</button>
                      )}
                      <button onClick={() => cancelAppointment(item._id)} className="w-full py-2 border border-red-200 text-red-400 rounded-xl text-sm hover:bg-red-500 hover:text-white transition-all">✕ Cancel</button>
                    </>
                  )}
                </div>
              </div>
              <div className="mt-4">{getStatusBadge(item)}</div>
            </div>
          ))}
        </div>
      )}
