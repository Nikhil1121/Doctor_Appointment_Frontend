    ...
    <div className="px-2 py-8">
      ...
      <div className="flex gap-3 mb-8 overflow-x-auto pb-2">
        {specialities.map((spec) => (
          <p key={spec} className={`cursor-pointer px-4 py-1 rounded-full border flex-shrink-0 ${spec === speciality ? 'bg-primary text-white border-primary' : 'text-gray-600 hover:bg-gray-100 border-gray-300'}`}
      ...
      <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
        {filterDoc.length > 0 ? filterDoc.map((item) => (
          <div onClick={() => navigate(`/appointment/${item._id}`)} className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:-translate-y-2 transition-all duration-500" key={item._id}>
